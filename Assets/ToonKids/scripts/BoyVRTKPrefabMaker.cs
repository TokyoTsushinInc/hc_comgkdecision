...
public class BoyVRTKPrefabMaker : MonoBehaviour
{
    ...
    public void Nexthaircolor(int todo)
    {
        ...
        if (hair == 6) SetGOmaterials(GOhair[6], MAThairA, 1, todo);
    }
    ...
    {
        if (chest == 0) SetGOmaterials(GOchest[0], MATshirt,0, todo);
        if (chest == 1) Setmaterials(GOchest, MATsweater, 0,todo);
        if (chest > 1) Setmaterials(GOchest, MATtshirt,0, todo);
    }
